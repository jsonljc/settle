  ...
  for (final entity in screensDir.listSync(recursive: true)) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (line.contains('GestureDetector') && !line.trim().startsWith('//')) {
        errors.add(
          '$path:$lineNum: GestureDetector used — use SettleTappable or wrap in Semantics',
        );
      }
      ...
      final sizedBoxMatch = RegExp(
        r'SizedBox\s*\(\s*(?:height|width)\s*:\s*(\d+)',
      ).firstMatch(line);
      if (sizedBoxMatch != null && !line.trim().startsWith('//')) {
        warnings.add(
          '$path:$lineNum: SizedBox with literal — prefer SettleGap',
        );
      }
    }
  }
